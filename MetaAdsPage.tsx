...
import {
  ...
  Rocket, Sparkles, ArrowRight, Award, User, Languages, ArrowLeft
} from 'lucide-react';
...
const WelcomeScreen = ({ onStart }: { onStart: () => void }) => {
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col font-sans relative overflow-x-hidden" dir={isAr ? "rtl" : "ltr"}>
      <Navbar />
      ...
      <div className="max-w-6xl mx-auto px-6 pt-32 pb-20 flex-1 flex flex-col items-center justify-center text-center z-10">
        ...
        >
          <div className={`relative p-8 rounded-[3rem] ${glassStyle} mb-10`}>
            ...
            <img
              ...
              className="w-32 h-32 md:w-44 md:h-44 object-contain drop-shadow-2xl relative z-10 transform transition-transform duration-500 hover:scale-110"
              referrerPolicy="no-referrer"
            />
          </div>

          <h1 className="text-4xl md:text-7xl font-black text-slate-900 mb-6 tracking-tight leading-tight">
            Meta Ads <span className="text-brand-blue drop-shadow-sm">Master Class</span>
          </h1>
          <p className="text-xl md:text-2xl text-slate-700 max-w-3xl mx-auto leading-relaxed font-bold">
            {isAr ? 'انضم لأقوى تجربة تفاعلية لاحتراف إعلانات Meta.' : 'The most powerful interactive experience to master Meta Ads.'}
          </p>
          <p className="text-lg text-slate-600 mt-4 max-w-2xl">
            {isAr ? 'تعلم بالممارسة، نافس نفسك، واستعن بالذكاء الاصطناعي ميديسترا.' : 'Learn by doing, challenge yourself, and use Medestra AI.'}
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl mb-16">
          <motion.div
            ...
            className={`p-10 rounded-[2.5rem] ${glassStyle} hover:-translate-y-3 transition-all duration-500 group cursor-default`}
          >
            <div className="w-16 h-16 bg-brand-blue/10 rounded-2xl flex items-center justify-center mb-6 text-brand-blue shadow-inner group-hover:bg-brand-blue group-hover:text-white transition-all duration-300">
              <LayoutDashboard className="w-8 h-8" />
            </div>
            <h3 className="font-black text-2xl mb-4 text-slate-900">{isAr ? 'تطبيق عملي' : 'Practical'}</h3>
            <p className="text-slate-600 text-lg leading-relaxed">{isAr ? 'محاكاة حقيقية لواجهة مدير الإعلانات لاتخاذ القرارات.' : 'Real Ads Manager simulation for decision making.'}</p>
          </motion.div>
          ...
          <motion.div
            ...
            className={`p-10 rounded-[2.5rem] ${glassStyle} hover:-translate-y-3 transition-all duration-500 group cursor-default`}
          >
            <div className="w-16 h-16 bg-purple-500/10 rounded-2xl flex items-center justify-center mb-6 text-purple-600 shadow-inner group-hover:bg-purple-600 group-hover:text-white transition-all duration-300">
              <Sparkles className="w-8 h-8" />
            </div>
            <h3 className="font-black text-2xl mb-4 text-slate-900">{isAr ? 'مدرب ذكي' : 'AI Tutor'}</h3>
            <p className="text-slate-600 text-lg leading-relaxed">{isAr ? 'شرح للمصطلحات، وتوجيه شخصي من المساعد الذكي.' : 'Term explanations and personal AI guidance.'}</p>
          </motion.div>
          ...
          <motion.div
            ...
            className={`p-10 rounded-[2.5rem] ${glassStyle} hover:-translate-y-3 transition-all duration-500 group cursor-default`}
          >
            <div className="w-16 h-16 bg-yellow-400/10 rounded-2xl flex items-center justify-center mb-6 text-yellow-500 shadow-inner group-hover:bg-yellow-400 group-hover:text-slate-900 transition-all duration-300">
              <Trophy className="w-8 h-8" />
            </div>
            <h3 className="font-black text-2xl mb-4 text-slate-900">{isAr ? 'شهادة إتقان' : 'Certificate'}</h3>
            <p className="text-slate-600 text-lg leading-relaxed">{isAr ? 'تجاوز التحديات لفتح المستويات والحصول على الشهادة.' : 'Pass challenges to unlock levels and earn your cert.'}</p>
          </motion.div>
        </div>
        ...
        <motion.button
          ...
          onClick={onStart}
          className="group relative px-12 py-5 bg-brand-blue text-white text-2xl font-black rounded-full shadow-[0_20px_50px_rgba(0,174,239,0.4),_inset_0_1px_1px_rgba(255,255,255,0.4)] hover:shadow-[0_25px_60px_rgba(0,174,239,0.5)] hover:-translate-y-1 transition-all duration-300 overflow-hidden"
        >
          <div className="flex items-center gap-4 relative z-10">
            <span>{isAr ? 'ابدأ كورس الاحتراف الآن' : 'Start Master Course Now'}</span>
            <Rocket className={`w-7 h-7 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform ${isAr ? '' : 'rotate-180'}`} />
          </div>
        </motion.button>

        <p className="mt-8 text-sm text-slate-500 font-bold uppercase tracking-widest">{isAr ? 'تحديثات Meta 2026 • الإصدار 1.0' : 'Meta 2026 Updates • v1.0'}</p>
      </div>
