import React from 'react';
import { motion } from 'framer-motion';
import { useLanguage } from '../LanguageContext';

const Beneficiaries: React.FC = () => {
  const { t } = useLanguage();

  // Previous: className="h-screen md:overflow-hidden snap-start pt-20 pb-4 md:pt-24 md:pb-8 bg-slate-50 border-t border-gray-100 flex flex-col justify-center"
  return (
    <section className="min-h-screen pt-20 pb-4 md:pt-24 md:pb-8 bg-transparent border-t border-gray-100 flex flex-col justify-center">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-4 md:mb-8">
          <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-2 md:mb-4">{t.beneficiaries.heading}</h2>
          <p className="text-base md:text-lg text-gray-600 max-w-4xl mx-auto px-2">
            {t.beneficiaries.subheading}
          </p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-4">
          {t.beneficiaries.items.map((item, index) => (
            <motion.div
              key={index}
              initial={{ opacity: 0, scale: 0.9 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              transition={{ delay: index * 0.1, duration: 0.4 }}
              className="flex flex-col items-center p-6 md:p-10 bg-white/[0.05] backdrop-blur-[40px] rounded-[3rem] border-t-[2.5px] border-l-[2.5px] border-white/90 border-b border-r border-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.1),_inset_0_1px_2px_rgba(255,255,255,0.9),_inset_0_-1px_1px_rgba(0,0,0,0.05)] hover:shadow-[0_30px_70px_rgba(0,0,0,0.15),_inset_0_1px_3px_rgba(255,255,255,1)] hover:-translate-y-3 transition-all duration-700 text-center group cursor-default relative overflow-hidden"
            >
              <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-50 text-brand-blue rounded-full flex items-center justify-center mb-2 md:mb-3 group-hover:bg-brand-blue group-hover:text-white transition-colors">
                <item.icon className="w-6 h-6 md:w-8 md:h-8" />
              </div>
              <span className="font-bold text-gray-800 text-sm md:text-base leading-tight">{item.label}</span>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default Beneficiaries;